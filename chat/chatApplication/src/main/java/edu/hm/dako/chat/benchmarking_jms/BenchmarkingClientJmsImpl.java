...
public class BenchmarkingClientJmsImpl extends JmsChatClient
        ...
        try {
            ...
            while (sharedClientData.status != ClientConversationStatus.REGISTERED) {
                ...
                if (sharedClientData.status == ClientConversationStatus.UNREGISTERED) {
                    ...
                    log.debug("User " + this.threadName + " schon im Server angemeldet");
                    return;
                }
            }
            ...
            // Derzeit werden in der Progrssbar Login und Logout nicht
            // beruecksichtigt
            // benchmarkingGui.countUpProgressTask();

            log.debug("User " + this.threadName + " beim Server angemeldet");
            ...
            while ((i < numberOfMessagesToSend) && (!userInterface.isTestAborted())) {
                ...
                i++;
                log.debug("Gesendete Chat-Nachrichten von " + this.threadName + ": " + i);
            }
            ...
            postLogout();
            log.debug("User " + threadName + " beim Server abgemeldet");
    ...
    private void waitUntilChatResponseReceived() {
        ...
        try {
            while (getLock() == true) {
                log.debug(this.threadName + " wartet auf Chat-Message-Response-PDU");
                Thread.sleep(1);
