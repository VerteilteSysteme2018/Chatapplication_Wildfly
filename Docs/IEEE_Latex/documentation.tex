...
Für die Umsetzung der Arbeit sollen verschiedene Lösungsansätze im Umfeld der verteilten Systeme untersucht, erprobt und bewertet werden. Dabei dient eine einfache Chat-Applikation als Grundlage, um die Funktionsweise zu verstehen und durch deren Weiterentwicklung verteilte Systeme auf Basis von \ac{MOM} in den Wissensstand übergehen zu lassen.

In den folgenden Kapiteln wird die Konzeption der Chat-Anwendung, deren genutzte Infrastruktur sowie die Umsetzung im Detail erklärt. Dabei wird auch auf die einzelnen Komponenten der Applikation sowie die Konfiguration der Serverschnittstellen eingegangen. Als abschließende Bewertung folgt ein Fazit. 
...
Um Nachrichten im Chatprozess zu speichern und zu protokollieren wird die Datenbank \textit{MariaDB} verwendet. Um die Anforderungen eines verteilten Systems gerecht zu werden, werden die Datenbankinstanzen in einem Dockercontainer\footnote{\url{https://www.docker.com/}} betrieben. Da Docker im Gegensatz zu einer virtuellen Maschine das Host-Betriebssystem mitbenutzt, können ressourcensparend leichtgewichtige, isolierte Container zur Verfügung gestellt werden, die aber dennoch über ein eigenes Dateisystem und Netzwerk-Interface verfügen \cite{Docker:Functionality}.
...
Wichtig ist hierbei die Verwendung der richtigen Syntax der jeweiligen wildfly-Version, da andernfalls die Verbindung zwischen Applikationsserver und Datenbank nicht erfolgreich hergestellt werden kann. 
...
Um eine übergreifende XA-Transaktion unter Einbeziehung der Zugriffe auf die JMS-Queue, das Topic sowie auf die Datenbanken zu realisieren übernimmt das \textit{Transactions subsystem} des Wildfly Application Servers die komplette Transaktionsverwaltung. Die beiden MariaDB-Container, welche in Docker ausgeführt werden, unterstützen den Prozess der XA-Transaktionen. Auch wurden die beiden Datenbanken als \textit{XA-Datasources} in der Konfigurationsdatei \textit{standalone.xml} des Application Servers definiert. 
...
Für das Chatten über JMS bzw. Kafka wurde die bereits vorhandene chatApplication in ein Maven Projekt umgewandelt und um eine neue Chatoberfläche ({\textit{ClientGUI}}) erweitert (vgl. Abbildung \ref{fig:clientGUI}). Diese wurde mit dem Java GUI-Toolkit Swing implementiert. Mithilfe der Klasse {\textit{GuiRunner}} ist es möglich eine Benutzeroberfläche in mehreren Threads zu starten. So kann der Chat Prozess zwischen verschiedenen Usern simuliert werden.
...
Grundlage für die Performance-Tests ist das bereits vorhandene, Java-basierte Benchmarking-Framework. Dieses wurde um eine JMS sowie eine Apache Kafka Anbindung erweitert um Leistungstest für das Versenden von Nachrichten über die beiden Message Broker durchzuführen.

Damit dies möglich ist, mussten mehrere Änderungen im Quellcode vorgenommen werden. Zunächst wurden zwei zusätzliche ImplementationTypes, \textit{JMSImplementation} und \textit{KafkaImplementation}, definiert. Desweiteren wurden die Klasse \textit{BenchmarkingClientFactory} um JMS spezifische Parameter erweitert.
...
Aufgrund kurzfristiger Änderungen in der Teamstruktur wurde für das Kafka Benchmarking eine eigene vereinfachte aber dennoch voll funktionstüchtige Benchmarking Methode entwickelt. Diese beinhaltet die Klassen \textit{KafkaChatClient}, \textit{KafkaReceiver} und \textit{KafkaTest}.

Die Anzahl der Clients, sowie die Menge an Nachrichten bzw. deren Byte-Größe kann verändert werden. Diese Einstellungen können für TCP in der Benutzeroberfläche, für JMS in den Klassen \textit{UserInterfaceInputParameters} und \textit{BenchmarkingUserInterfaceSimulation} bzw. für Kafka in den Klassen \textit{KafkaReceiver} und \textit{KafkaTest} angepasst werden.
...
Vergleicht man beide Testdurchläufe wird deutlich, dass sich durch die Umstellung von JMS auf Kafka die RTT des Chat Prozesses stark erhöht. Wärhend bei JMS Nachrichten selbst bei einer Anzahl an 50 Clients Nachrichten noch unter einer Sekunde verschickt werden, sind die RTT Zeiten bei Kafka selbst bei einer geringen Anzahl an Clients bereits im höheren Sekundenbereich.
\\
\\
...
\subsubsection{Unterschiede zu ReST}
